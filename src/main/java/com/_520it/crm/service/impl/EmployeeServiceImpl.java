...
public class EmployeeServiceImpl implements IEmployeeService {
    ...
    public int insert(Employee record) {
        ...
        record.setStatus(Employee.STATUS_NORMAL);
    ...
    public PageResult queryPage(EmployeeQueryObject qo) {
        ...
        //判断是否是经理
        if(!subject.hasRole("HR")){//若不是经理,则设置负责人id
            Employee employee = (Employee) subject.getPrincipal();
            Long currentId = employee.getId();
            qo.setCurrentId(currentId);
    ...
    @Override
    //修改密码
    public void savePsw(String username, String newpsw) {
        employeeMapper.updatePsw(username,newpsw);
    }
